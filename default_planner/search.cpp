    ...
    MemoryPool& mem, int start, int goal, Neighbors* ns)
{
    ...
    while (open.size() > 0){
        ...
        for (int i=0; i<4; i++){
            ...
            if (env->past_waitings[curr->id*5+i+1].second != 0)
                cost += (double)env->past_waitings[curr->id*5+i+1].first/(double)env->past_waitings[curr->id*5+i+1].second;
    ...
    MemoryPool& mem, int start, unordered_set<int> goals, Neighbors* ns)
{
    ...
    while (open.size() > 0){
        ...
        // if (curr->id == goal)
        if (goals.find(curr->id) != goals.end())
        {
            goal_node = curr;
            break;
        }
    ...
    if (goal_node == nullptr){
        std::cout << "error in multi astar: no path found "<< start<< std::endl;
        assert(false);
        exit(1);
    }
